...
* ECMWF's Polytope is an open source library ([GitHub - ecmwf/polytope: A library for extracting polytope "features" from datacubes](https://github.com/ecmwf/polytope)) for extracting complex data from datacubes. Its API enables any arbitrary n-dimensional polygon (called a polytope) to be extracted from a datacube, allowing for efficient extraction of complex features, such as polygon regions or spatio-temporal paths. Like OGC API-EDR, Polytope can encode data in OGC CoverageJSON [^covjson]. Polytope also mirrors the query patterns of OGC API-EDR: feature extraction allows users to request standard meteorological features such as time series, vertical profiles, and arbitrary polygons, retrieving only the data they need rather than downloading global fields. Polytope is typically deployed on top of ECMWF's FDB and GribJump technology, and this stack is currently deployed in a variety of contexts:
  * Polytope is used to serve ECMWF core operational data from its 24/7 weather forecasting pipeline. This instance (polytope.ecmwf.int) currently serves its member states and will enter into operations in late 2026.
  * In Destination Earth, Polytope is deployed in multiple datacentres across Europe, serving high-resolution data from the Global Extremes Digital Twin, the On-Demands Extreme Digital Twin (regional) and the Climate Digital Twin. In this context it currently serves around 50TiB of data per day, and enables feature extraction on vast quantities of weather and climate data.
  * MeteoSwiss deploys an instance of this software stack on their own infrastructure, designed to efficiently serve their Open Data offering. Their forecast data on regular lat-lon and ICON grids is stored in FDB and served with Polytope feature extraction.
  * Polytope is also able to extract from XArray/Zarr instead of FDB/GRIB, and this technology is being developed further to offer Polytope as a plugin to pygeoapi.
